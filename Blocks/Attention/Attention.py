...
class AttentionLayer(nn.Module):
    ...
    - mask: 可接受多种形状的掩码，会根据输入的形状进行广播：
        -  (batch_size, q_len, k_len) 每个查询对应一个掩码向量，指示哪些键是有效的
        -  (batch_size, k_len) 所有查询共享一个掩码向量，指示哪些键是有效的【在序列中比较常见】

    输出:
    - out: (batch_size, q_len, hidden_dim)
    """

    def __init__(self, q_dim, k_dim, v_dim, hidden_dim, attention_type="scaled_dot"):
        super().__init__()
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def __init__(self, q_dim, k_dim, v_dim, hidden_dim, num_heads=8, attention_type="scaled_dot"):
        super().__init__()
        ...
        if attention_type == "dot":
            ...
            self.attention = ScaledDotProductAttention(scale=self.head_dim ** -0.5)
        else:
            raise ValueError(f"Unsupported attention type: {attention_type}")
