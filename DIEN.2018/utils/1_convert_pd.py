    ...
    with open_fn(file_path, "rt", encoding="utf-8") as fin:
        for line in fin:
            ...
            try:
                ...
                # Some Amazon files are python-literal dict strings.
                records.append(ast.literal_eval(line))
    return pd.DataFrame(records)


def resolve_input(raw_data_dir: Path, stem: str) -> Path:
    json_path = raw_data_dir / f"{stem}.json"
    gz_path = raw_data_dir / f"{stem}.json.gz"
    if json_path.exists():
        return json_path
    if gz_path.exists():
        return gz_path
    raise FileNotFoundError(f"Missing {stem}.json or {stem}.json.gz in {raw_data_dir}")
